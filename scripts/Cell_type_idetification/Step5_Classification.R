...
# merge by cluster
cL_resutls = do.call("rbind", lapply(unique(seurat.integrated@meta.data$RNA_snn_res.0.9), function(cl){
  es.max.cl = sort(rowSums(es.max[ ,rownames(seurat.integrated@meta.data[seurat.integrated@meta.data$RNA_snn_res.0.9==cl, ])]), decreasing = !0)
  head(data.frame(cluster = cl, type = names(es.max.cl), scores = es.max.cl, ncells = sum(seurat.integrated@meta.data$RNA_snn_res.0.9==cl)), 10)
}))
...
for(j in unique(sctype_scores$cluster)){
  ...
  seurat.integrated@meta.data$customclassif[seurat.integrated@meta.data$RNA_snn_res.0.9 == j] = as.character(cl_type$type[1])
}

pdf(file = "classifaction.pdf", width = 10, height = 10) 
DimPlot(seurat.integrated, reduction = "umap", label = TRUE, repel = TRUE, group.by = 'customclassif')        
dev.off()
...
ccolss= c("#5f75ae","#92bbb8","#64a841","#e5486e","#de8e06","#eccf5a","#b5aa0f","#e4b680","#7ba39d","#b15928","#ffff99", "#6a3d9a","#cab2d6","#ff7f00","#fdbf6f","#e31a1c","#fb9a99","#33a02c","#b2df8a","#1f78b4","#a6cee3","#5f75ae","#92bbb8","#64a841","#e5486e")
...
pdf(file = "classifaction_bubble.pdf", width = 15, height = 15)
